...
function collectOutline() {
  headings = [...document.querySelectorAll<HTMLElement>(".markdown-body h2[id], .markdown-body h3[id]")];
...
<template>
  ...
  <main class="wrap markdown-post-layout">
    ...
    <aside v-if="outline.length" ref="panel" class="markdown-outline pixel" :style="panelStyle" aria-label="文章目录，可拖动边框移动" @pointerdown="startDrag" @pointermove="moveDrag" @pointerup="endDrag" @pointercancel="endDrag" @lostpointercapture="endDrag">
      ...
      <nav><a v-for="item in outline" :key="item.id" :href="`#${item.id}`" :aria-current="activeId === item.id ? 'location' : undefined" :class="{ nested: item.level === 3, active: activeId === item.id }">{{ item.text }}</a></nav>
    </aside>
  </main>
